...
While 1
	...
	Switch $hMsg
		Case $GUI_EVENT_CLOSE
			Exit
		Case $hButton
			If (GUICtrlRead($hInput_pinpai1) And GUICtrlRead($hInput_pinpai2) And GUICtrlRead($hInput_shichang) == "") Then
				MsgBox(0, "错误提示", "请输入各项数值，输入不能为空！")
			Else
				$result_pinpai1 = $hInput_pinpai1 * $hInput_shichang * 365*10 / 1000/2 ;完整公式为 ： 额定功率*每天使用时长*365天*10年/1000瓦时*0.5电费
				$result_pinpai2 = $hInput_pinpai2 * $hInput_shichang * 3650 / 2000 ;完整公式为 ： 额定功率*每天使用时长*365天*10年/1000瓦时*0.5电费
				$result_string = '品牌一，十年共计电费' & $result_pinpai1 & '元 人民币' & @CRLF & '品牌二，十年共计电费' & $result_pinpai2 & '元人民币' & @CRLF
